...
function dateLabel(value: string) {
  const date = new Date(
    value.length <= 10 ? `${value.slice(0, 10)}T00:00:00` : value,
  );
    ...
    <section className="mt-7">
      ...
      {items.length === 0 ? (
        <div className="rounded-[1.8rem] border border-border/60 bg-card/80 px-5 py-6 text-center shadow-soft">
          <Music2 className="mx-auto size-5 text-primary" strokeWidth={1.5} />
          <p className="mt-3 text-sm font-medium">時間線還在等第一首歌</p>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
        ...
        <div className="relative ml-2 border-l border-primary/20 pl-5">
          {items.map((item, index) => (
            <article
              key={item.id}
              className={index === items.length - 1 ? "relative pb-1" : "relative pb-6"}
            >
              <span
                aria-hidden
              ...
              <div className="rounded-[1.65rem] border border-border/60 bg-card/85 px-4 py-4 shadow-soft backdrop-blur-xl">
                ...
                {item.songs.length > 0 ? (
                  <div className="mt-3 space-y-2">
                    {item.songs.map(({ role, song }, songIndex) => {
