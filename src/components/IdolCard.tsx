...
export function IdolCard({
  ...
  return (
    <article className="polaroid relative overflow-hidden">
      <Link
        to="/idols/$idolId"
      ...
      >
        ...
        <div className="px-1.5 pt-2.5">
          <div className="flex items-center gap-1.5">
            <p className="font-display min-w-0 truncate text-[18px] font-semibold">{idol.name}</p>
            {isMain ? <span className="shrink-0 text-[11px] text-primary">♡</span> : null}
          </div>
      ...
      {onSetMain ? (
        <button
          ...
          onClick={onSetMain}
          disabled={isMain}
          className={`mt-3 flex w-full items-center justify-center gap-1.5 rounded-full border px-2 py-2 text-[11px] font-medium transition-colors ${
            isMain
...
export function EmptySlot({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex aspect-[4/5] w-full flex-col items-center justify-center gap-2 rounded-[1.5rem] border border-dashed border-primary/25 bg-surface/35 text-muted-foreground transition-transform duration-300 active:scale-[0.98]"
    >
