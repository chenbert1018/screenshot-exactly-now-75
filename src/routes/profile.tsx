...
function ProfilePage() {
  ...
  return <AppShell>
    ...
    <Section title="提醒設定"><SoftCard className="px-5 py-5"><p className="flex items-center gap-2 text-[15px] font-medium">🔔 重要日子提醒</p><p className="mt-1 text-sm text-muted-foreground">已開啟的提醒會同步到 App，並在 iPhone 排程通知。</p>{reminderRows.length === 0 ? <p className="mt-4 rounded-2xl bg-surface/60 px-4 py-4 text-sm text-muted-foreground">還沒有提醒。到日子或偶像頁面就能設定。</p> : <ul className="mt-4 divide-y divide-border/60">{reminderRows.map((row) => <li key={row.id} className="flex items-center gap-3 py-3"><div className="min-w-0 flex-1"><p className="truncate text-sm">{row.title}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{row.idolName} · {formatDaysBefore(row.daysBefore)}</p></div><Switch checked={row.enabled} aria-label={`${row.title} 提醒開關`} onCheckedChange={(v) => void updateReminder(row.id, { enabled: v })} /><button type="button" aria-label={`刪除 ${row.title} 提醒`} onClick={() => void removeReminder(row.id)} className="rounded-full p-2 text-muted-foreground"><Trash2 className="size-4" /></button></li>)}</ul>}</SoftCard></Section>
